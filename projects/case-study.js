(() => {
  ...
  const sharedAgenticIntro = "The pipeline, checks, data model, and reports should work without AI. AI is optional and can only help explain results or prepare questions.";
  ...
  const companionBlogs = {
    "ai-data-reliability": ["The Pipeline Succeeded. Why Was the Data Still Wrong?", "A successful job status can still hide stale inputs, duplicate records, schema drift, or reconciliation failure.", "../blog/pipeline-succeeded-data-wrong.html", "Engineering blueprint · 7 min"],
    "pipeline-optimization": ["Faster Is Not Better If the Data Changes", "Why performance work begins with a reproducible workload and a correctness gate.", "../blog/faster-is-not-better.html", "Engineering blueprint · 6 min"],
    "industrial-service-intelligence": ["From Service Tickets to a Trusted Industrial Data Product", "How grain, conformed context, and metric definitions turn fragmented service records into an operational product.", "../blog/service-tickets-trusted-data-product.html", "Case-study blueprint · 6 min"],
    "robotic-fleet-optimization": ["Why the Nearest Robot Is Not Always the Best Robot", "A transparent comparison of availability-only assignment and health-aware scheduling.", "../blog/nearest-robot-not-best.html", "Research blueprint · 6 min"],
    "retail-inventory-intelligence": ["A Dashboard Is Not a Data Product", "Why modeling, source grain, and metric definitions must come before charts.", "../blog/dashboard-is-not-data-product.html", "Future case study · 5 min"],
    "manufacturing-root-cause": ["AI Can Suggest a Cause. It Should Not Declare One.", "A manufacturing investigation that keeps hypotheses, evidence, contradictions, and human review visible.", "../blog/ai-suggest-cause-not-declare.html", "Research blueprint · 7 min"],
    "supply-chain-digital-twin": ["Simulate the Decision Before You Make It", "A digital twin is useful when it compares controlled decisions—not when it is only a visualization.", "../blog/simulate-before-decision.html", "Research blueprint · 6 min"],
    "database-performance-lab": ["When Does an Index Help the Workload, Not Just One Query?", "Move from a single faster query to workload-level evidence across latency, throughput, writes, and storage.", "../blog/index-helps-workload.html", "Research blueprint · 6 min"],
    "business-case-studies": ["From Architecture Diagram to Business Value", "Translate one technical intervention into a measurable decision, benefit hypothesis, adoption risk, and evidence plan.", "../blog/architecture-to-business-value.html", "Case-study blueprint · 5 min"]
  };
     ...
     business, and research content remains centralized in this JavaScript file. */
  {
    ...
    const blog = companionBlogs[projectId];
    const companionBlog = blog ? `<section class="case-study-section companion-blog-section"><div class="case-shell companion-blog-card"><div><p class="case-kicker">Companion blog</p><span>${blog[3]}</span><h2>${blog[0]}</h2><p>${blog[1]}</p></div><a class="button button-primary" href="${blog[2]}">Read the story / case study</a></div></section>` : "";
    ...
    root.innerHTML = `
      ...
      <main id="case-main">
        ...
        <section class="case-study-section exists-section" id="current-state"><div class="case-shell"><div class="case-section-heading"><p>10 · Current state</p><h2>What Exists Today</h2></div><div class="exists-grid"><article><span>Implemented</span>${list(existsToday.implemented.length ? existsToday.implemented : ["Nothing is implemented yet."])}</article><article><span>Improving</span>${list(existsToday.improving.length ? existsToday.improving : ["More testing and documentation are needed."])}</article><article><span>Future</span>${list(existsToday.future)}</article></div></div></section>
        ...
        <section class="case-study-section experiment-section" id="experiment-plan"><div class="case-shell"><div class="case-section-heading"><p>12 · Evaluation plan</p><h2>Experiment Plan and Evidence</h2></div><div class="experiment-plan-grid"><article><span>Hypothesis</span><p>${profile.experiment.hypothesis}</p></article><article><span>Independent variables</span>${list(profile.experiment.variables)}</article><article><span>Acceptance boundary</span><p>Compare like-for-like workloads, preserve correctness checks, record hardware and configuration, and reject improvements that weaken data quality.</p></article></div><div class="case-section-heading evidence-subheading"><p>Evidence inventory</p><h3>Inspectable assets</h3></div><div class="evidence-grid case-evidence-grid" id="evidence">${project.evidence.map(([name, status, description]) => `<article><span class="evidence-status evidence-${status.toLowerCase().replaceAll(" ", "-")}">${status}</span><h3>${name}</h3><p>${description}</p></article>`).join("")}</div></div></section>

        <section class="case-study-section case-shell" id="larger-scale"><div class="case-section-heading"><p>13 · Larger workloads</p><h2>What Breaks at Scale?</h2></div><p class="scale-intro">The local design is not production-ready. These are the first parts that would need to change.</p><ol class="scale-risk-list">${scaleRisks.map((risk, index) => `<li><span>${String(index + 1).padStart(2, "0")}</span><p>${risk}</p></li>`).join("")}</ol></section>
        ...
        <section class="case-study-section case-shell" id="agentic-ai"><div class="case-section-heading"><p>15 · Later AI work</p><h2>Optional AI Assistance</h2></div><blockquote class="agent-principle">Code and data checks establish the facts. AI can help explain them.</blockquote><div class="agentic-ai-panel"><ol class="agentic-flow" aria-label="Reviewed AI workflow">${agenticFlow.map((step) => `<li>${step}</li>`).join("")}</ol><div class="agent-plane-grid"><article><h3>Data work</h3>${list(["ingestion", "transformations", "quality", "models", "deterministic actions"])}</article><article><h3>AI help</h3>${list(["reasoning", "retrieval", "planning", "recommendations", "tools"])}</article><article><h3>Controls</h3>${list(["allowed / blocked actions", "permissions", "risk level", "approval", "execution limit"])}</article><article><h3>Records</h3>${list(["prompt and model version", "evidence", "tool call", "decision / approval", "latency / cost"])}</article></div></div><div class="case-section-heading safety-heading"><p>Failure tests</p><h3>Expected behavior when something goes wrong</h3></div><div class="agent-safety-grid">${agentSafety.map(([test, expected]) => `<article><h4>${test}</h4><p>${expected}</p></article>`).join("")}</div></section>
  ...
  root.innerHTML = `
    ...
    <main id="case-main">
      ...
      <section class="case-study-section exists-section" id="current-state">
        <div class="case-shell">
          <div class="case-section-heading"><p>04 · Current state</p><h2>What Exists Today</h2></div>
          <div class="exists-grid">
            <article><span>Implemented</span>${list(existsToday.implemented.length ? existsToday.implemented : ["Nothing is implemented yet."])}</article>
      ...
      <section class="case-study-section case-shell" id="evidence">
        ...
        <p class="evidence-intro">Each item is marked as available, in progress, or planned.</p>
